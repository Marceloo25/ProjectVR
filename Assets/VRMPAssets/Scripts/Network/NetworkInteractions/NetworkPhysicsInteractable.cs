...
namespace XRMultiplayer
{
    ...
    public class NetworkPhysicsInteractable : NetworkBaseInteractable
    {
        ...
        public override void OnNetworkSpawn()
        {
            ...
            // Ensure forwarding state matches ownership at spawn
            SetForwardingEnabled(IsOwner);
        }
        ...
        public override void OnGainedOwnership()
        {
            ...
            if (IsOwner)
            {
                ...
                // Enable local forwarders and hit sources for the new owner
        ...
        public override void OnLostOwnership()
        {
            ...
            m_RequestingOwnership = false;
            m_ClientNetworkTransform.enabled = true;
            // Disable local forwarders and hit sources for non-owners
            SetForwardingEnabled(false);
        }

        void SetForwardingEnabled(bool enabled)
        {
            // Toggle LocalColliderForwarder and IHitSource-like components under this object.
            // We avoid compile-time dependency on types from other assemblies by inspecting
            // components by runtime type name / implemented interfaces.
            var behaviours = GetComponentsInChildren<Behaviour>(true);
            foreach (var b in behaviours)
            {
                var t = b.GetType();
                // LocalColliderForwarder by type name
                if (t.Name == "LocalColliderForwarder")
                {
                    try { b.enabled = enabled; } catch { }
                    continue;
                }

                // Any behaviour that implements an interface named IHitSource
                var ifs = t.GetInterface("IHitSource");
                if (ifs != null)
                {
                    try { b.enabled = enabled; } catch { }
                }
