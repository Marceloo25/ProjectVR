...
namespace XRMultiplayer
{
    ...
    public class NetworkMessageBoard : NetworkBehaviour
    {
        ...
        void Start()
        {
            ...
            messageList = new NetworkList<FixedString512Bytes>(default, NetworkVariableReadPermission.Everyone, NetworkVariableWritePermission.Owner);
        }
        ...
        public void SubmitTextLocal(string text)
        {
            ...
            // Simple command parsing: if message starts with '/' treat as command
            if (text.StartsWith("/"))
            {
                // Split on whitespace: command is first token, rest is args
                ...
                if (cmd == "/scene")
                {
                    ...
                    {
                        // Inform local user about usage
                        CreateText($"<b>System</b>: Usage: /scene [sceneName]");
                        return;
                    }

                    // Only the server/host should execute scene changes for everyone
                    if (NetworkManager.Singleton != null && NetworkManager.Singleton.IsServer)
                    {
                        // Request all clients to load the named scene
                        var fs = new FixedString128Bytes(arg);
                        LoadSceneClientRpc(fs);
                        // Also load locally on the host
                        try { SceneManager.LoadScene(arg); } catch (Exception e) { CreateText($"<b>System</b>: Failed to load scene '{arg}': {e.Message}"); }

                        // Broadcast a system message indicating the change
                        string announce = $"<b>System</b>: Host changed scene to <i>{arg}</i>";
                        var announceFs = new FixedString512Bytes(announce);
                        SubmitMessageOwnerRpc(announceFs);
                    }
                    else
                    {
                        CreateText("<b>System</b>: Only the host can change scenes.");
                    }
            ...
            SubmitMessageOwnerRpc(newText);
        }

        [ClientRpc]
        void LoadSceneClientRpc(FixedString128Bytes sceneName, ClientRpcParams rpcParams = default)
        {
            // Called on clients by the server/host to load a scene
            try
            {
            ...
            {
                // If scene load fails, show a local system message
                CreateText($"<b>System</b>: Failed to load scene '{sceneName}': {e.Message}");
            }
        }
        ...
        [Rpc(SendTo.Owner)]
        void SubmitMessageOwnerRpc(FixedString512Bytes text)
        {
        ...
        /// <param name="text">The message to submit.</param>
        [Rpc(SendTo.Everyone)]
        void SubmitMessageRpc(FixedString512Bytes text)
        {
